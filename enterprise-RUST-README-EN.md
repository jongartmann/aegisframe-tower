...
Hardware-attested PSCP proof stack on AWS EC2 with NVIDIA GPU — Rust Edition.

---

## What This Stack Proves

When AegisFrame blocks a prompt, this stack provides **three independent hardware/OS-level proofs** that no AI inference occurred:

| Proof | Source | Level | Rust Module |
|-------|--------|-------|-------------|
...
│   │   ├── capture_before()   → Snapshot of all 3 sources
│   │   ├── capture_after()    → Snapshot after decision
│   │   ├── produce_proof()    → Combined PSCP_PROOF
...
│   │   └── GpuAttestor::attest()    → Delta Comparison → Verdict
│   │
│   ├── socket.rs           # Socket Monitor [feature = "ebpf"]
│   │   ├── SocketMonitor::new()              → Check eBPF availability
│   │   ├── SocketMonitor::snapshot()         → /proc/net/tcp + eBPF Trace
│   │   └── SocketMonitor::attest_no_outbound() → Connection Delta
│   │
│   └── process.rs          # Process Attestation
│       ├── ProcessAttestor::new()       → /proc or sysinfo Init
│       ├── ProcessAttestor::snapshot()  → Scan inference signatures
│       └── ProcessAttestor::attest()    → Process Delta → Verdict
```

### Conditional Compilation

```rust
// gpu.rs — only compiled with --features gpu
#[cfg(feature = "gpu")]
...
#[cfg(not(feature = "gpu"))]
pub struct GpuAttestor;  // Stub: always "UNAVAILABLE"
```
...
### Advantages vs. Python Docker

| Aspect | Python Image | Rust Image |
|--------|-------------|------------|
| Image Size | ~1.2 GB | ~200 MB |
| Startup | ~2s (Gunicorn Fork) | ~50ms |
| Memory | ~80 MB | ~8 MB |
| Layers | 6 (CUDA + Python + pip + code) | 3 (CUDA + binary + static) |
| Attack Surface | Python + pip + all deps | Static binary |
...
## API Examples with curl + JSON

### Generate PSCP Hardware Proof
...
The socket monitor watches these known model inference API endpoints:
...
Any outbound connection to these endpoints during a PSCP BLOCK would set the verdict to `OUTBOUND_DETECTED`.
